...
public class Tuberculoz : Mover
{
    ...
    static int GROUND_Y = Data.GAME_HEIGHT - 1 - 446;
    ...
    Tuberculoz(string reference) : base(reference)
    {
        ...
        fl_immune = false;
        immuneTimer = 0;
        recents = new Dictionary<int, bool>();
        fl_death = false;
        fl_defeated = false;
    ...
    void Flag(int uid)
    {
        if (recents.ContainsKey(uid - _firstUniq))
        {
            recents[uid - _firstUniq] = true;
        }
        else
        {
            recents.Add(uid - _firstUniq, true);
        }
    }
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            do
            {
                b.MoveTo(
                    ...
                    Data.GAME_HEIGHT - Random.Range(0, 150) - 100
                );
                fl_tooClose = false;
    ...
    void UpdateTornado()
    {
        ...
        if (Random.Range(0, 3) > 0)
        {
            ...
            for (var i = 0; i < l.Count; i++)
            {
                ...
                {
                    var wind = Random.Range(0, 22) / 10.0f + 0.5f;
                    if (e.fl_stable)
                    {
    ...
    {
        // Roches
        if ((dx != 0 | dy != 0 | (next != null && next.action == DIE)) & Random.Range(0, 3) == 0)
        {
            if (Random.Range(0, 2) == 0)
            {
                game.fxMan.InGameParticles(Data.PARTICLE_STONE, Random.Range(0, Data.GAME_WIDTH), Data.GAME_HEIGHT, Random.Range(0, 2));
            }
            else
            {
                game.fxMan.InGameParticles(Data.PARTICLE_STONE, Random.Range(0, Data.GAME_WIDTH), Data.GAME_HEIGHT - Random.Range(0, Mathf.RoundToInt(Data.GAME_HEIGHT * 0.6f)), Random.Range(0, 2));
            }
        ...
        // Strikes
        if (next != null && next.action == DIE)
        {
            dx *= game.xFriction;
            if (Random.Range(0, 3) == 0)
            {
                HammerAnimation a;
                a = game.fxMan.AttachFx(Data.GAME_WIDTH * 0.5f, y + Random.Range(0, 60), "hammer_fx_strike");
                PlayAnim(Data.ANIM_BOSS_HIT);
                ReplayAnim();
    ...
    void OpenExit()
    {
        ...
        SetLayer(Data.DP_SPRITE_BACK_LAYER);
        var cloak = SpecialItem.Attach(game, Data.GAME_WIDTH * 0.5f, 40, 113, null);
        cloak.dy = 20;

        var tubkey = ScoreItem.Attach(game, Data.GAME_WIDTH * 0.5f - 40, 40, 199, null);
        tubkey.dy = 25;

        var door = game.world.view.AttachSprite("door", game.FlipCoordReal(Entity.x_ctr(5)), Entity.y_ctr(18), true);
    ...
    public void OnKillBad()
    {
        ...
        if (badKills >= B_STEP)
        {
            ...
            itemB = SpecialItem.Attach(game, Data.GAME_WIDTH - 31, Data.GAME_HEIGHT, 5, null);
        ...
        else
        {
            if (badKills == A_STEP)
            {
                ...
                itemA = SpecialItem.Attach(game, 31, Data.GAME_HEIGHT, 4, null);
        ...
        {
            var it = SpecialItem.Attach(game, 214, Data.GAME_HEIGHT - 340, 36, null);
            it.SetLifeTimer(0);
        }
    }
    ...
    public override void EndUpdate()
    {
        ...
        if(game.mc.united==null)
        {
            Debug.Log("PAS NORMAL");
            Debug.Log(game);
        }
        // Link la barre de vie aux tremblements du jeu
        if (lifeBar != null && game.mc.united!=null)
        {
            lifeBar._x = game.mc._x;
